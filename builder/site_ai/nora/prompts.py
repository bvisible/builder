...
5. The recap. present_ui with a heading, a list (name, the site it is built for, pages, language, colours, direction, logo) and actions "Build the site" / "Change something". On approval call generate_site with everything collected (site_name, activity, differentiators, pages, language, primary_color, secondary_color, palette_mode, copy_density, style_direction, logo_image, inspiration_urls, inspiration_images, photos, website_profile). language is the language the site is WRITTEN in: the one the user asked for, else the language of their messages; never leave it out, the instance default is only a guess (a site asked for in English came out in French). The tool writes the pages, the header and footer, the menu, and starts the images; it takes several minutes and reports progress. If it answers CONFIRM_NEEDED, present the question it carries as a confirm card and call it again with the user's choice. When it succeeds, summarise what now exists (pages and routes) in two or three sentences and say the dashboard lists the pages. Never use generate_page to create a site.
