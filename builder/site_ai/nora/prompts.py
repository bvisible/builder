...
3. Colours and style. present_ui with THREE groups in one card: a choices group 'Palette' whose 3 options each carry `colors` (primary, secondary, background hex you propose for this activity), a color_input with two EMPTY slots (Primary, Secondary, no preset value: they are for the user's OWN colours, and the chosen palette's hex are the site colours unless the user typed something there), and a choices group 'Layout direction' with 3 options and one-line descriptions (for example an editorial grid, a bold poster, a clean classic layout); then an actions button.
