...
class TestTokens(unittest.TestCase):
	def test_prefix_is_the_initials_or_the_first_word(self):
		self.assertEqual(token_prefix("Blue Ridge Supply"), "brs")
		self.assertEqual(token_prefix("The 5 Ravens"), "t5r")
...
class TestShopIncludes(unittest.TestCase):
	...
	def test_other_business_compares_the_site_name_with_the_instance_company(self):
		...
		defaults[("Website Profile", "Burrows", "title")] = "a B2C storefront"
		with patch("builder.site_ai.nora.site_builder.frappe.db.get_value", side_effect=lambda d, n, f: defaults.get((d, n, f))), patch(
			...
			self.assertFalse(_other_business("Burrows", "a B2C storefront"))
...
class TestVisualCheck(unittest.TestCase):
	...
	def test_loopback_url_names_the_profile(self):
		...
		with patch.object(visual_check.frappe, "local") as local, patch.object(visual_check.frappe, "conf", {"webserver_port": 8000}):
			...
			# //// Neoffice — assert the loopback URL targets 127.0.0.1 and the site is named via header, not host (0445cc94 "fix(visual-check): the loopback render names its site by header, not by host")
			...
			# //// Neoffice — same 127.0.0.1 loopback target as above (0445cc94 "fix(visual-check): the loopback render names its site by header, not by host")
...
# //// Neoffice ▼▼▼ — new: tests covering a reseller site build fixes (2d78d71d "fix(nora): includes written as offered, routes honoured but home, and the build's routes stated as final")
